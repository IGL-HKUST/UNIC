...
def read_topology(file, encoding):
	...
	for t in T:
		...
		# Face
			...
			idx = [n.split('/') for n in t.replace('f ', '').replace('\n', '').split(' ')]
			vti = [int(n[0]) - 1 for n in idx]
			vni = [int(n[-1]) - 1 for n in idx]
			F += [vti]
			Vni += [vni]
	...
	Ft = np.array(Ft, np.int32)
	Vn = np.array(Vn, np.float32)
	Vn = Vn / np.linalg.norm(Vn, axis=-1, keepdims=True)
	Vni = np.array(Vni, np.int32)

	if Ft.shape: assert F.shape[0] == Ft.shape[0], 'Inconsistent .obj file, mesh and UV map do not have the same number of faces' 
	else: Vt, Ft = None, None
	
	return V, F, Vt, Ft, Vn, Vni